...
lint:
	...
	-pep8 beet_playlist tests
	pylint beet_playlist tests

test: test-deps
	python setup.py nosetests

test-cover: test-deps
	python setup.py nosetests --with-coverage --cover-package=beet_playlist
...
coverage: test-deps
	coverage run --source beet_playlist setup.py nosetests
	make coverage-show
...
docs: docs-deps
	rm -f docs/beet_playlist.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ beet_playlist
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
